...
static int subWord(uint8_t *dword)
{
    return subBytes(dword, 4, false);
}
    ...
    else
    {
        ...
        for (size_t rowIndex = n; rowIndex < (n + 7) * 4; ++rowIndex)
        {
            if (!(rowIndex % n))
            {
                ...
                memcpy(previousRow, roundKeys + DWORD_ROW_INDEX_TO_OFFSET(rowIndex - 1), 4);

                rotWord(previousRow);
                int e;
                e = subWord(previousRow);
                if (e)
                    return e;
